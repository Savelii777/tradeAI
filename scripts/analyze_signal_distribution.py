#!/usr/bin/env python3
"""Check how many signals pass threshold per pair over last N candles."""

import pandas as pd
import numpy as np
import joblib
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).parent.parent))
sys.path.insert(0, str(Path(__file__).parent))

from train_mtf import MTFFeatureEngine
from src.utils.constants import CUMSUM_PATTERNS, ABSOLUTE_PRICE_FEATURES

def add_volume_features(df):
    df = df.copy()
    df['vol_sma_20'] = df['volume'].rolling(20).mean()
    df['vol_ratio'] = df['volume'] / df['vol_sma_20']
    df['vol_zscore'] = (df['volume'] - df['vol_sma_20']) / df['volume'].rolling(20).std()
    return df

def calculate_atr(df, period=14):
    tr = pd.concat([
        df['high'] - df['low'],
        abs(df['high'] - df['close'].shift()),
        abs(df['low'] - df['close'].shift())
    ], axis=1).max(axis=1)
    return tr.ewm(span=period, adjust=False).mean()

def prepare_features(data, mtf_fe):
    m1, m5, m15 = data['1m'], data['5m'], data['15m']
    
    for df in [m1, m5, m15]:
        if not isinstance(df.index, pd.DatetimeIndex):
            df.index = pd.to_datetime(df.index, utc=True)
        df.sort_index(inplace=True)
    
    ft = mtf_fe.align_timeframes(m1, m5, m15)
    ft = ft.join(m5[['open', 'high', 'low', 'close', 'volume']])
    ft = add_volume_features(ft)
    ft['atr'] = calculate_atr(ft)
    ft = ft.dropna(subset=['close', 'atr'])
    
    cols_to_drop = [c for c in ft.columns if any(p in c.lower() for p in CUMSUM_PATTERNS)]
    ft = ft.drop(columns=cols_to_drop, errors='ignore')
    
    absolute_cols = [c for c in ft.columns if c in ABSOLUTE_PRICE_FEATURES]
    ft = ft.drop(columns=absolute_cols, errors='ignore')
    
    return ft.ffill().dropna()

def main():
    model_dir = Path(__file__).parent.parent / "models" / "v8_improved"
    dir_model = joblib.load(model_dir / 'direction_model.joblib')
    timing_model = joblib.load(model_dir / 'timing_model.joblib')
    strength_model = joblib.load(model_dir / 'strength_model.joblib')
    features = joblib.load(model_dir / 'feature_names.joblib')
    
    data_dir = Path(__file__).parent.parent / "data" / "candles"
    mtf_fe = MTFFeatureEngine()
    
    # Thresholds
    MIN_CONF = 0.57
    MIN_TIMING = 1.3
    MIN_STRENGTH = 1.7
    
    LAST_N = 2000  # Last N 5-minute candles (~7 days)
    
    all_pairs = [
        'BTC_USDT_USDT', 'ETH_USDT_USDT', 'SOL_USDT_USDT', 
        'XRP_USDT_USDT', 'DOGE_USDT_USDT', 'ZEC_USDT_USDT',
        'ADA_USDT_USDT', 'LINK_USDT_USDT', 'AVAX_USDT_USDT',
        'PIPPIN_USDT_USDT', 'ASTER_USDT_USDT'
    ]
    
    print(f"Analyzing last {LAST_N} candles per pair...")
    print(f"Thresholds: Conf>{MIN_CONF}, Tim>{MIN_TIMING}, Str>{MIN_STRENGTH}")
    print(f"\n{'Pair':22s} | Conf>0.57 | Tim>1.3 | Str>1.7 | ALL | Avg ATR%")
    print("-" * 75)
    
    total_signals = 0
    
    for pair in all_pairs:
        try:
            m1 = pd.read_parquet(data_dir / f'{pair}_1m.parquet')
            m5 = pd.read_parquet(data_dir / f'{pair}_5m.parquet')
            m15 = pd.read_parquet(data_dir / f'{pair}_15m.parquet')
            
            ft = prepare_features({'1m': m1, '5m': m5, '15m': m15}, mtf_fe)
            
            # Take last N rows
            data = ft.tail(LAST_N).copy()
            n = len(data)
            
            for f in features:
                if f not in data.columns:
                    data[f] = 0.0
            
            X = data[features].values.astype(np.float64)
            X = np.nan_to_num(X, nan=0.0)
            
            probas = dir_model.predict_proba(X)
            max_probs = np.max(probas, axis=1)
            timing = timing_model.predict(X)
            strength = strength_model.predict(X)
            
            # Count signals
            conf_pass = (max_probs > MIN_CONF).sum()
            timing_pass = (timing > MIN_TIMING).sum()
            strength_pass = (strength > MIN_STRENGTH).sum()
            all_pass = ((max_probs > MIN_CONF) & (timing > MIN_TIMING) & (strength > MIN_STRENGTH)).sum()
            
            avg_atr = data['m5_atr_14_pct'].mean() if 'm5_atr_14_pct' in data.columns else 0
            
            total_signals += all_pass
            
            print(f"{pair:22s} | {conf_pass:4d} ({conf_pass/n*100:4.1f}%) | {timing_pass:4d} ({timing_pass/n*100:4.1f}%) | {strength_pass:4d} ({strength_pass/n*100:4.1f}%) | {all_pass:3d} | {avg_atr:.4f}")
            
        except Exception as e:
            print(f"{pair:22s} | Error: {e}")
    
    print("-" * 75)
    print(f"Total potential signals across all pairs: {total_signals}")
    print(f"Expected trades per day (if scanning every 5min): {total_signals / 7:.1f}")

if __name__ == "__main__":
    main()
